...
def calcConf(freqSet, H, supportData, brl, minConf=0.05):  # 规则生成与评价
...
def rulesFromConseq(freqSet, H, supportData, brl, minConf=0.05):
    '''
...
def generateRules(L, supportData, minConf=0.05):
    '''
...
def sql_news(start, end):
    ...
    try:
        ...
        sql = "SELECT b.title FROM work.news_type as a join work.news as b on a.id_news = b.uuid " \
              "where b.date > '%s' and b.date < '%s' and a.type = '公司新闻';" % (start, end)
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    data = sql_news("2017-5-26", "2017-6-27")  # 导入数据集
    ...
    L, suppData = apriori(myDat, 0.01)  # 选择频繁项集
