...
if __name__ == "__main__":
    ...
    Path2 = "C:/Users/text/Desktop/"
    Sentences = load_sent(Path2 + 'pinglun_clear2.txt')

    SentimentWords = load_words(Path1 + 'sentimentwords2.txt')
    ...
    for item in Sentences.items():
        ...
        with open(Path2 + 'sentiment_result.txt', 'a') as f2:

            f2.write(ind + ' ' + str(Score) + '\n')

        if Count == 0:
            with open(Path2 + 'count.txt', 'a') as f3:
                f3.write(ind + '\n')
